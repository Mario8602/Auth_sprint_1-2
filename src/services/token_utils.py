...
class TokenUtil:
    ...
    def create_tokens(
        ...
        with tracer.start_as_current_span("TokenUtil.create_tokens"):
            access_token = self.create_token(
                ...
                timedelta(minutes=settings.access_token_expire_time)
            )
            refresh_token = self.create_token(
                base_payload,
                self.TOKEN_TYPE_REFRESH,
                timedelta(minutes=settings.refresh_token_expire_time)
            )
    ...
    def create_token(
            ...
            base_payload: dict,
            token_type: str,
            exp_timedelta: timedelta | None = timedelta(minutes=10)
    ) -> str:
