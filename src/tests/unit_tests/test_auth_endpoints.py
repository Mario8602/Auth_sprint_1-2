...
async def test_old_access_token_after_refresh(
    api_client: AsyncClient, simple_user: User
):
    user = simple_user
    login_data = {
        "username": user.username,
...
async def test_login_history(api_client: AsyncClient, simple_user: User):
    ...
    result = await api_client.post(
        "/api/v1/auth/refresh_token", headers=refresh_headers
    )
...
async def test_reset_username(api_client: AsyncClient, simple_user: User):
    ...
    assert result.status_code == HTTPStatus.OK
    assert (
        result.json() == "Username was successfully changed to: username='new_username'"
    )
