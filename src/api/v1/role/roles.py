...
async def create_role(role_data: RoleCreate,
                      ...
                      auth_credentials: HTTPAuthorizationCredentials = Depends(oauth2_scheme)) -> RoleResponse:
    with tracer.start_as_current_span("create_role"):
        try:
            ...
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                                detail='The permission was not found')
        except DuplicateEntityException as ex:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                                detail=str(ex))


@router.get('/read/{role_id}',
            response_model=RoleResponse,
            summary='Viewing a role',
            description='Viewing a role by id.')
...
async def update_role(role_id: UUID, role_data: RoleUpdate,
    ...
    with tracer.start_as_current_span("update_role"):
        try:
            response = await role_service.update_role(role_id, role_data,
...
@router.get('/permissions',
            ...
            description='View all permissions that exist in the database.')
async def get_all_permissions(role_service: RoleService = Depends(get_role_service),
                              auth_credentials: HTTPAuthorizationCredentials = Depends(oauth2_scheme)):
