...
async def oauth_redirect(
        ...
        expires_in: str,
        provider_name: str,
        oauth_service: BaseOauthService = Depends(get_oauth_service)
):
...
async def login(
    ...
    with tracer.start_as_current_span("login"):
        ...
        try:
            resp = await auth_service.login(form_data, user_agent)
...
@router.put(
    ...
    description="Changes user password to specified"
)
...
async def verify_token(
    ...
    try:
        ...
        return AuthUser(**auth_user_data)
